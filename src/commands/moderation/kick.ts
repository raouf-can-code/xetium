...
export default {
  ...
  callback: async (client: Client, int: any) => {
    ...
    if (targetUser.id === int.guild.ownerId) {
      await int.editReply("trying to kick the server owner is a great idea");
      return;
    }
    ...
    if (int.user.id === int.guild.ownerId) {
      try {
        ...
        await int.editReply(`${targetUser} was kicked from the server\nreason: ${reason}`);
      } catch (err) {
        console.error(err);
    ...
    if (targetUserRolePosition >= requestUserRolePosition) {
      await int.editReply("you can't kick that user because he/she has same/higher role then you");
      return;
    }
    if (targetUserRolePosition >= botRolePosition) {
      await int.editReply("I can't kick that user because he/she has same/higher role then me");
      return;
    }
